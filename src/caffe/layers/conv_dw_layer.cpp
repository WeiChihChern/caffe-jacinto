...
void ConvolutionDepthwiseLayer<Ftype,Btype>::LayerSetUp(
      const vector<Blob*>& bottom, const vector<Blob*>& top) {
  ConvolutionParameter conv_param = this->layer_param_.convolution_param();

  CHECK(bottom[0]->channels() == conv_param.num_output() && conv_param.num_output() == conv_param.group()) <<
          "For Depthwise Seperable Convolution, input channels, output channels and groups must have same value. " <<
          bottom[0]->channels() << " " << conv_param.num_output() << " " << conv_param.group();
...
template<typename Ftype, typename Btype>
void ConvolutionDepthwiseLayer<Ftype,Btype>::Reshape(
      const vector<Blob*>& bottom, const vector<Blob*>& top) {
  ...
  caffe_set(weight_multiplier_.count(), Btype(1),
  ...
  if (this->layer_param_.convolution_param().bias_term()) {
    ...
    caffe_set(bias_multiplier_.count(), Btype(1),
          bias_multiplier_.mutable_cpu_data());
  }
}

template<typename Ftype, typename Btype>
void ConvolutionDepthwiseLayer<Ftype,Btype>::Forward_cpu(
      const vector<Blob*>& bottom, const vector<Blob*>& top) {
    ...
    for (int c = 0; c < channels; ++c) {
      for (int h = 0; h < top_height; ++h) {
        for (int w = 0; w < top_width; ++w) {
          const Ftype* weight_data = weight_data_base
...
template<typename Ftype, typename Btype>
void ConvolutionDepthwiseLayer<Ftype,Btype>::Backward_cpu(
