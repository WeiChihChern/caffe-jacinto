...
template <typename Ftype>
__global__ void AvePoolForwardQuantize(const int nthreads,
    const Ftype* const bottom_data, const int num, const int channels,
    const int height, const int width, const int pooled_height,
    const int pooled_width, const int kernel_h, const int kernel_w,
    const int stride_h, const int stride_w, const int pad_h, const int pad_w,
    Ftype* const top_data, const int qscale_in) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    const int pw = index % pooled_width;
    const int ph = (index / pooled_width) % pooled_height;
    const int c = (index / pooled_width / pooled_height) % channels;
    ...
    for (int h = hstart; h < hend; ++h) {
      for (int w = wstart; w < wend; ++w) {
        aveval += int(bottom_slice[h * width + w] * qscale_in * mult_factor + 0.5);
      }
    }

    // top_data[index] = aveval / pool_size;
    aveval = aveval / float(qscale_in*qscale);
    top_data[index] = aveval;
  }
}
...
void PoolingLayer<Ftype, Btype>::Forward_gpu(const vector<Blob*>& bottom,
  ...
  case PoolingParameter_PoolMethod_AVE:
    if(quantize) {
        // NOLINT_NEXT_LINE(whitespace/operators)
        AvePoolForwardQuantize<Ftype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream>>>(
            count, bottom_data, bottom[0]->num(), channels_,
            height_, width_, pooled_height_, pooled_width_, kernel_h_,
            kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_, top[0]->mutable_gpu_data<Ftype>(), qscale_in);
    } else {
        // NOLINT_NEXT_LINE(whitespace/operators)
