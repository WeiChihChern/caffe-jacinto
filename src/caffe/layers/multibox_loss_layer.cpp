...
void MultiBoxLossLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
  ...
  GetPriorBBoxes(prior_data, num_priors_, &prior_bboxes, &prior_variances,4); //TODO need to enhance for num_keypoint for more than 0

  // Retrieve all predictions.
  vector<LabelBBox> all_loc_preds;
  GetLocPredictions(loc_data, num_, num_priors_, loc_classes_, share_location_,
                    &all_loc_preds,0); // same function is shared for key points flow, hence passing zero here
