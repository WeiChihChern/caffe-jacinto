...
void ClipBBox(const NormalizedBBox& bbox, NormalizedBBox* clip_bbox) {
  ...
  for (int32_t i = 0; i < clip_bbox->keypoint_x_size(); i++){
    clip_bbox->set_keypoint_x(i,std::max(std::min(bbox.keypoint_x(i), 1.f), 0.f));
    clip_bbox->set_keypoint_y(i,std::max(std::min(bbox.keypoint_y(i), 1.f), 0.f));
  }
...
void ClipBBox(const NormalizedBBox& bbox, const float height, const float width,
  ...
  for (int32_t i = 0; i < clip_bbox->keypoint_x_size(); i++){
    clip_bbox->set_keypoint_x(i,std::max(std::min(bbox.keypoint_x(i), width), 0.f));
    clip_bbox->set_keypoint_y(i,std::max(std::min(bbox.keypoint_y(i), height), 0.f));
...
void ScaleBBox(const NormalizedBBox& bbox, const int height, const int width,
  ...
  for (int32_t i = 0; i < scale_bbox->keypoint_x_size(); i++){
    scale_bbox->set_keypoint_x(i, bbox.keypoint_x(i) * width);
    scale_bbox->set_keypoint_x(i, bbox.keypoint_y(i) * height);
  }
...
bool ProjectBBox(const NormalizedBBox& src_bbox, const NormalizedBBox& bbox,
  ...
  for (int32_t i = 0; i < proj_bbox->keypoint_x_size(); i++){
    proj_bbox->set_keypoint_x(i, (bbox.keypoint_x(i) - src_bbox.keypoint_x(i)) / src_width);
    proj_bbox->set_keypoint_y(i, (bbox.keypoint_y(i) - src_bbox.keypoint_y(i)) / src_height);
  }
...
void ExtrapolateBBox(const ResizeParameter& param, const int height,
  ...
  if (height_scale > 0 && width_scale > 0 &&
    ...
    for (int32_t i = 0; i < bbox->keypoint_x_size(); i++){
      bbox->set_keypoint_x(i, bbox->keypoint_x(i) * crop_width / width_scale);
      bbox->set_keypoint_y(i, bbox->keypoint_y(i) * crop_height / height_scale);
    }
...
void EncodeBBox(
  ...
  const int code_size_ = 4; // TODO to update code size for num_keypoint
  if (code_type == PriorBoxParameter_CodeType_CORNER) {
    if (encode_variance_in_target) {
      ...
      CHECK_EQ(prior_variance.size(), code_size_);
...
void DecodeBBox(
  ...
  if (code_type == PriorBoxParameter_CodeType_CORNER) {
    ...
    float decode_bbox_x = 0.0;
    float decode_bbox_y = 0.0;
    ...
    if (variance_encoded_in_target) {
      ...
      for(int32_t i = 0; i < bbox.keypoint_x_size(); i++){
        ...
        decode_bbox_x =
            prior_variance[4 + 2*i] * bbox.keypoint_x(i) * prior_width + prior_center_x;
        decode_bbox_y =
            prior_variance[4 + 2*i + 1] * bbox.keypoint_y(i) * prior_height + prior_center_y;

        decode_bbox->add_keypoint_x(decode_bbox_x);    
        decode_bbox->add_keypoint_y(decode_bbox_y);
      }
    }
...
void DecodeBBoxes(
  ...
  if (num_bboxes >= 1) {
    CHECK_EQ(prior_variances[0].size(), (bboxes[0].keypoint_x_size()*2+4));
  }
  decode_bboxes->clear();
...
void MineHardExamples(const Blob& conf_blob,
  ...
  *num_negs = 0;
  ...
  int top_k = -1;
  const int code_size_ = 4; // TODO need to take care for num_keypoint >0
  if (has_nms_param) {
  ...
  if (mining_type == MultiBoxLossParameter_MiningType_HARD_EXAMPLE) {
    ...
    if (*num_matches != 0) {
      ...
      loc_shape[1] = *num_matches * code_size_;
...
void GetLocPredictions(const Dtype* loc_data, const int num,
      ...
      const bool share_location, vector<LabelBBox>* loc_preds, const int num_keypoint) {
  loc_preds->clear();
  if (share_location) {
  ...
  int code_size_ = num_keypoint*2 + 4;

  for (int i = 0; i < num; ++i) {
    LabelBBox& label_bbox = (*loc_preds)[i];
    for (int p = 0; p < num_preds_per_class; ++p) {
      int start_idx = p * num_loc_classes * code_size_;
      for (int c = 0; c < num_loc_classes; ++c) {
        int label = share_location ? -1 : c;
        if (label_bbox.find(label) == label_bbox.end()) {
          label_bbox[label].resize(num_preds_per_class);
        }
        label_bbox[label][p].set_xmin(loc_data[start_idx + c * code_size_]);
        label_bbox[label][p].set_ymin(loc_data[start_idx + c * code_size_ + 1]);
        label_bbox[label][p].set_xmax(loc_data[start_idx + c * code_size_ + 2]);
        label_bbox[label][p].set_ymax(loc_data[start_idx + c * code_size_ + 3]);
        
        label_bbox[label][p].clear_keypoint_x();
        label_bbox[label][p].clear_keypoint_y();

        for(int i=0; i < num_keypoint; i++){
          label_bbox[label][p].add_keypoint_x(loc_data[start_idx + c * code_size_ + 4 + 2*i]);
          label_bbox[label][p].add_keypoint_y(loc_data[start_idx + c * code_size_ + 4 + 2*i + 1]);
        }

      }
    }
    loc_data += num_preds_per_class * num_loc_classes * code_size_;
  }
}
...
void EncodeLocPrediction(const vector<LabelBBox>& all_loc_preds,
  ...
  const bool use_prior_for_matching =
      multibox_loss_param.use_prior_for_matching();
  int count = 0;
  int code_size_ = 4; // Need to upgrade to num_keypoint*2 + 4
  ...
  for (int i = 0; i < num; ++i) {
    for (map<int, vector<int> >::const_iterator
      ...
      for (int j = 0; j < match_index.size(); ++j) {
        ...
        loc_gt_data[count * code_size_] = gt_encode.xmin();
        loc_gt_data[count * code_size_ + 1] = gt_encode.ymin();
        loc_gt_data[count * code_size_ + 2] = gt_encode.xmax();
        loc_gt_data[count * code_size_ + 3] = gt_encode.ymax();
        ...
        if (bp_inside) {
          ...
          loc_pred_data[count * code_size_] =
              (match_bbox.xmin() < 0 || match_bbox.xmin() > 1) ?
              gt_encode.xmin() : loc_pred[j].xmin();
          loc_pred_data[count * code_size_ + 1] =
              (match_bbox.ymin() < 0 || match_bbox.ymin() > 1) ?
              gt_encode.ymin() : loc_pred[j].ymin();
          loc_pred_data[count * code_size_ + 2] =
              (match_bbox.xmax() < 0 || match_bbox.xmax() > 1) ?
              gt_encode.xmax() : loc_pred[j].xmax();
          loc_pred_data[count * code_size_ + 3] =
              (match_bbox.ymax() < 0 || match_bbox.ymax() > 1) ?
              gt_encode.ymax() : loc_pred[j].ymax();
        } else {
          loc_pred_data[count * code_size_] = loc_pred[j].xmin();
          loc_pred_data[count * code_size_ + 1] = loc_pred[j].ymin();
          loc_pred_data[count * code_size_ + 2] = loc_pred[j].xmax();
          loc_pred_data[count * code_size_ + 3] = loc_pred[j].ymax();
        }
        if (encode_variance_in_target) {
          for (int k = 0; k < code_size_; ++k) {
            CHECK_GT(prior_variances[j][k], 0);
            loc_pred_data[count * code_size_ + k] /= prior_variances[j][k];
            loc_gt_data[count * code_size_ + k] /= prior_variances[j][k];
          }
        }
        ++count;
      }
...
void ComputeLocLoss(const TBlob<Dtype>& loc_pred, const TBlob<Dtype>& loc_gt,
  ...
  const int code_size_ = 4; // need to take care num_keypoint >0
  if (loc_count != 0) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (map<int, vector<int> >::const_iterator
      ...
      for (int j = 0; j < match_index.size(); ++j) {
        ...
        Dtype loss = 0;
        for (int k = 0; k < code_size_; ++k) {
          Dtype val = diff_data[count * code_size_ + k];
...
void GetPriorBBoxes(const Dtype* prior_data, const int num_priors,
      ...
      vector<vector<float> >* prior_variances, const int code_size_) {
  prior_bboxes->clear();
  prior_variances->clear();
  
  for (int i = 0; i < num_priors; ++i) {
    int start_idx = i * code_size_;
    NormalizedBBox bbox;
  ...
  for (int i = 0; i < num_priors; ++i) {
    int start_idx = (num_priors + i)*code_size_ ;
    vector<float> var;
  ...
  //LOG(INFO) << "variance is fetched till address  " << &prior_data[(num_priors + num_priors)*code_size_];
...
void GetDetectionResults(const Dtype* det_data, const int num_det,
      ...
      map<int, map<int, vector<NormalizedBBox> > >* all_detections, const int num_keypoint) {
  all_detections->clear();
  int output_size = 7+2*num_keypoint;
  for (int i = 0; i < num_det; ++i) {
    int start_idx = i * output_size;
...
void VisualizeBBox(const vector<cv::Mat>& images, const Blob* detections,
                   ...
                   const string& save_file, const int num_keypoint) {
  // Retrieve detections.
  const int num_det = detections->height();
  const int num_img = images.size();
  const int num_keypoint_ = 0; // TODO to enchance for num_keypoint > 0
  const int output_size = 7 + num_keypoint*2;
  CHECK_EQ(detections->width(), output_size);
  ...
  for (int i = 0; i < num_det; ++i) {
    const int img_idx = detections_data[i * output_size];
    CHECK_LT(img_idx, num_img);
    const int label = detections_data[i * output_size + 1];
    const float score = detections_data[i * output_size + 2];
    if (score < threshold) {
      continue;
    }
    NormalizedBBox bbox;
    bbox.set_xmin(detections_data[i * output_size + 3] * width);
    bbox.set_ymin(detections_data[i * output_size + 4] * height);
    bbox.set_xmax(detections_data[i * output_size + 5] * width);
    bbox.set_ymax(detections_data[i * output_size + 6] * height);
    bbox.set_score(score);

    for (int j = 0; j < num_keypoint_; j++){
      bbox.set_keypoint_x(j, detections_data[i * output_size + 7 + 2*j] * width);
      bbox.set_keypoint_y(j, detections_data[i * output_size + 7 + 2*j + 1] * height);
    }
    ...
    for (map<int, vector<NormalizedBBox> >::iterator it =
      ...
      for (int j = 0; j < bboxes.size(); ++j) {
        ...
        for(int k = 0; k < num_keypoint_; k++){
          cv::Point key_pt0(bboxes[j].keypoint_x(k), bboxes[j].keypoint_y(k));
          cv::circle(image, key_pt0, 2, color, 4);
        }
      }
    }
